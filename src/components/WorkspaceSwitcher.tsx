  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 bg-white/80 hover:bg-white transition-all text-xs font-semibold shadow-sm"
      >
        <div className="w-6 h-6 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-100 flex items-center justify-center shrink-0">
      ...
      {isOpen && (
        <div className="absolute left-0 mt-2 w-64 glass-panel rounded-2xl z-[120] py-2 overflow-hidden">
                ...
                >
                  <div className="min-w-0 pr-2">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className={`text-[11px] font-bold px-1.5 py-0.5 rounded border ${wsRole.color}`}>
                        {wsRole.label}
                      </span>
                      {ws.state && <span className="text-[11px] text-slate-500 font-mono">{ws.state}</span>}
                    </div>
                  </div>
          ...
          <div className="pt-2 border-t border-slate-100 px-2">
            ...
            >
              <Plus className="w-4 h-4 text-emerald-600" /> Add an organization
            </button>
          </div>
        </div>
      )}
